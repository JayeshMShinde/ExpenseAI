  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast({
        title: 'Parsing Error',
        description:
          error instanceof Error ? error.message : `Failed to parse ${selectedFile.name}. Please check the file format or try again.`,
        variant: 'destructive',
      });
      onTransactionsParsed([]);
      setIsLoading(false);
  ...
  return (
    <Card className="shadow-md h-full border border-border/70 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-card to-card/95">
      <CardHeader className="pb-3">
        ...
        <CardDescription>Select a CSV or PDF bank statement file to analyze your expenses.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 pt-1">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center p-6 text-center pointer-events-none">
            <ArrowUpCircle className={`h-10 w-10 mb-3 ${isDragging ? 'text-primary animate-bounce' : 'text-muted-foreground'}`} />
          ...
          <div className="flex items-center justify-between rounded-md border border-border/60 bg-secondary/70 p-3 transform transition-all duration-200 hover:bg-secondary/90">
            <div className="flex items-center gap-3 text-sm overflow-hidden">
              ...
              <div className="overflow-hidden">
                <p className="font-medium truncate text-foreground/90" title={selectedFile.name}>
                  {selectedFile.name}
                </p>
                <p className="text-xs text-muted-foreground">
                  {(selectedFile.size / 1024).toFixed(1)} KB · {selectedFile.type === 'text/csv' ? 'CSV' : 'PDF'}
                </p>
              </div>
            </div>
            <Button
